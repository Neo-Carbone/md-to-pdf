#!/usr/bin/env python3
"""
Markdown to PDF Converter - Standalone App
Can be used from command line or as a GUI app.
"""
...
def _validate_input_path(input_path: Path) -> None:
    ...
    if not input_path.exists() or not input_path.is_file():
        raise FileNotFoundError("Input file not found or is not a file.")
    ...
    if output_path.exists():
        ...
        if not ALLOW_OVERWRITE:
            raise FileExistsError(
                "Output file already exists. Set MD2PDF_ALLOW_OVERWRITE=1 to allow replacing it."
            )
...
"""
        ...
        print("Error converting file: An unexpected internal error occurred.", file=sys.stderr)
        return False

def main():
    """Main entry point."""
    # Check if running as GUI app (no arguments) or command line
    if len(sys.argv) < 2:
        ...
        try:
            ...
            # No GUI available, show usage
            print(__doc__)
            print("\nUsage:")
            print("  convert_md_to_pdf input.md [output.pdf]")
            print("\nOr drag and drop a .md file onto this app.")
            sys.exit(1)
    else:
        # Command line usage
